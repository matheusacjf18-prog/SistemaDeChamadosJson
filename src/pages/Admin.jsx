...
const Admin = () => {
    const [abaAtiva, setAbaAtiva] = useState('ListaAvisos');
            ...
            <div className="row">
                <div className="col-md-3 mb-4">
                    <div className="list-group shadow-sm">
                        <button className={`list-group-item list-group-item-action ${abaAtiva === 'avisos' ? 'active' : ''}`} onClick={() => setAbaAtiva('avisos')}>📢 Mural de Avisos</button>
                        <button className="list-group-item list-group-item-action text-primary" onClick={() => navigate('/cadastro-clientes')}>👥 Clientes</button>
                        <button className="list-group-item list-group-item-action text-danger" onClick={handleLogout}>Sair</button>
                    </div>
                </div>
                <div className="col-md-9">
                    <div className="card shadow-sm p-4">
                        {abaAtiva === 'ListaAvisos' && <AdminListaAvisos session={session} />}
                        ...
                        {abaAtiva === 'avisos' && <ListaAvisos session={session} />}
                    </div>
                </div>
            </div>
        </div>
...
const AdminListaAvisos = ({ session }) => {
    const [ListaAvisos, setListaAvisos] = useState([]);
    
    const carregar = () => fetch(`${API_URL}/admin/ListaAvisos`).then(r => r.json()).then(d => { if(d.success) setListaAvisos(d.ListaAvisos) });
    useEffect(() => { carregar(); }, []);
...
const ListaAvisos = ({ session }) => {
    const [avisos, setAvisos] = useState([]);
    const [loading, setLoading] = useState(true);
    const [titulo, setTitulo] = useState('');
    const [mensagem, setMensagem] = useState('');

    const JSON_SERVER_URL = 'http://localhost:5000/avisos';
    ...
    const carregarAvisos = async () => {
        try {
            ...
            console.error("Erro ao carregar avisos da API fake:", error);
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleSubmit = async (e) => {
        ...
        const novoAviso = {
            titulo,
            mensagem,
            autor: session?.full_name || 'Usuário Desconhecido',
        ...
        <div>
            <div className="card mb-4 border-0 shadow-sm bg-light">
                <div className="card-body">
                    <h5 className="card-title mb-3">Publicar Novo Aviso</h5>
                    <form onSubmit={handleSubmit}>
                        <div className="mb-3">
                            <label className="form-label text-muted small fw-bold">TÍTULO DO AVISO</label>
                            <input type="text" className="form-control" value={titulo} onChange={e => setTitulo(e.target.value)} required />
                        </div>
                        <div className="mb-3">
                            <label className="form-label text-muted small fw-bold">MENSAGEM</label>
                            <textarea className="form-control" rows="3" value={mensagem} onChange={e => setMensagem(e.target.value)} required></textarea>
                        </div>
                        <button type="submit" className="btn btn-primary w-100">Publicar no Mural</button>
                    </form>
                </div>
            </div>

            <h5 className="border-bottom pb-2 mb-3">Mural de Avisos</h5>
            {avisos.length === 0 ? (
                <p className="text-center text-muted">Nenhum aviso publicado ainda.</p>
            ) : (
                    ...
                    {avisos.slice().reverse().map(aviso => (
                        <div key={aviso.id} className="col-12">
                            <div className="card shadow-sm border-start border-4 border-info">
                                <div className="card-body">
                                    <h6 className="card-title fw-bold text-dark">{aviso.titulo}</h6>
                                    <p className="card-text text-secondary mb-3">{aviso.mensagem}</p>
                                    <div className="d-flex justify-content-between align-items-center border-top pt-2 mt-2">
                                        <small className="text-muted"><i className="bi bi-person-circle me-1"></i> {aviso.autor}</small>
