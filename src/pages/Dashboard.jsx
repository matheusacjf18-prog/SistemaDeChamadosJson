            ...
            <div className="row">
                <div className="col-md-3 mb-4">
                    ...
                    <div className="list-group shadow-sm">
                        ...
                        <button className={`list-group-item list-group-item-action ${view === 'ListaAvisos' ? 'active' : ''}`} onClick={() => setView('ListaAvisos')}>📋 Meus ListaAvisos</button>
                ...
                <div className="col-md-9">
                    <div className="card p-4 shadow-sm">
                        ...
                        {view === 'abrir-chamado' && <FormChamado session={session} onViewTickets={() => setView('ListaAvisos')} />}
                        {view === 'ListaAvisos' && <ListaListaAvisos session={session} />}
...
const ListaListaAvisos = ({ session }) => {
    const [ListaAvisos, setListaAvisos] = useState([]);
    const [loading, setLoading] = useState(true);

    useEffect(() => {
        fetch(`${API_URL}/ListaAvisos?user=${session.login}`)
            .then(r => r.json())
            .then(d => { if (d.success) setListaAvisos(d.ListaAvisos); setLoading(false); })
            .catch(() => setLoading(false));
    }, [session.login]);

    if (loading) return <div className="text-center py-5"><div className="spinner-border text-primary"></div></div>;
    if (ListaAvisos.length === 0) return <p className="text-center text-muted">Nenhum chamado aberto.</p>;

    return ListaAvisos.map(t => (
